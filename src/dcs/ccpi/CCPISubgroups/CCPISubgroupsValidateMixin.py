...
class CCPISubgroupsValidateMixin:
    ...
    def check(date_str, field, expected, actual):
        if abs(expected - actual) > TOLERANCE:
            raise ValueError(
                f"{date_str}: {field} "
                f"expected ~{expected * 100:.1f}%, got {actual * 100:.1f}%"
            )
    ...
    @classmethod
    def validate_subgroup(cls, d_list, subgroup):
        values = [d[f"{subgroup}_ccpi"] for d in d_list]
        cls.validate_change_month_to_month(
            d_list, values, f"{subgroup}_change_month_to_month"
        )
        cls.validate_inflation_year_to_year(
            d_list, values, f"{subgroup}_inflation_year_to_year"
        )
        cls.validate_inflation_12mma(
            d_list, values, f"{subgroup}_inflation_12_month_moving_average"
        )
    ...
    def validate_d_list(cls, d_list):
        ...
        for subgroup in cls.SUBGROUP_LABELS:
            cls.validate_subgroup(d_list, subgroup)
