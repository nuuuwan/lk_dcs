...
class ValidateMixin:
    ...
    def validate_key(cls, d_list, key):
        values = [d[f"{key}_ccpi"] for d in d_list]
        cls.validate_change_month_to_month(
            d_list, values, f"{key}_ccpi_change_month_to_month"
        )
        cls.validate_inflation_year_to_year(
            d_list, values, f"{key}_inflation_year_to_year"
        )
        cls.validate_inflation_12mma(
            d_list, values, f"{key}_inflation_12_month_moving_average"
        )
