...
class CCPICoreValidateMixin:
    ...
    @classmethod
    def validate_change_month_to_month(cls, d_list, ccpi_core):
        for i in range(1, len(d_list)):
            actual = d_list[i]["change_month_to_month"]
            if None in (ccpi_core[i], ccpi_core[i - 1], actual):
                continue
            expected = (
                (ccpi_core[i] - ccpi_core[i - 1]) / ccpi_core[i - 1] * 100
            )
            cls.check(
                d_list[i]["date_str"],
    ...
    @classmethod
    def validate_inflation_year_to_year(cls, d_list, ccpi_core):
        for i in range(12, len(d_list)):
            actual = d_list[i]["inflation_year_to_year"]
            if None in (ccpi_core[i], ccpi_core[i - 12], actual):
                continue
            expected = (
                (ccpi_core[i] - ccpi_core[i - 12]) / ccpi_core[i - 12] * 100
            )
            cls.check(
                d_list[i]["date_str"],
    ...
    @classmethod
    def validate_inflation_12mma(cls, d_list, ccpi_core):
        for i in range(23, len(d_list)):
            actual = d_list[i]["inflation_12_month_moving_average"]
            a, b, c = i - 23, i - 11, i + 1
            prev, recent = ccpi_core[a:b], ccpi_core[b:c]
    ...
    def validate_d_list(cls, d_list):
        ...
        ccpi_core = [d["ccpi_core"] for d in d_list]
        cls.validate_months(dates)
        cls.validate_change_month_to_month(d_list, ccpi_core)
        cls.validate_inflation_year_to_year(d_list, ccpi_core)
        cls.validate_inflation_12mma(d_list, ccpi_core)
